...
describe('usersController', () => {
    ...
    it('show', async () => {
        await createUser();
        const getUserResponse = await request.get('/api/users').set('Authorization', 'Bearer ' + token);
        let id = getUserResponse.body[0].id;
        const response = await request.get(`/api/users/${id}`).set('Authorization', 'Bearer ' + token);
